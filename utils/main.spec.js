...
describe('scraper-instagram', () => {
        ...
        (SESSION_ID ? describe : describe.skip)('By session ID', () => {
            ...
            describe('Get own profile', () => {
                ...
                it('gets profile', async () =>
                    expect((profile = await client.getProfile())).to.be.an('object'));
                it('has access', () => expect(profile.access).to.be.true);
            });
            describe('Account', () => {
                it('gets notifications', () =>
                    expect(client.getAccountNotifications()).to.be.fulfilled);
                it('gets stories', () =>
                    expect(client.getAccountStories()).to.be.fulfilled).timeout(5000);
            });
        });
    });
    describe('Get profile', () => {
        ...
        (PUBLIC_PROFILE ? describe : describe.skip)('Get public profile', () => {
            let profile;
            it('gets profile', async () =>
                expect((profile = await client.getProfile(PUBLIC_PROFILE))).to.be.an('object'));
            it('has access', () => expect(profile.access).to.be.true);
        });
        (PRIVATE_PROFILE ? describe : describe.skip)('Get private profile', () => {
            describe('Without authentication', () => {
                let profile;
                it('gets profile', async () =>
                    expect((profile = await client.getProfile(PRIVATE_PROFILE))).to.be.an(
                        'object'
                    ));
    ...
    (SESSION_ID ? describe : describe.skip)('Get profile story', () => {
        ...
        (STORY_PROFILE_ID ? it : it.skip)(
            'gets story by ID',
            () => expect(client.getProfileStoryById(STORY_PROFILE_ID)).to.be.fulfilled
        ).timeout(5000);
        (STORY_PROFILE_USERNAME ? it : it.skip)(
            'gets story by username',
            () => expect(client.getProfileStory(STORY_PROFILE_USERNAME)).to.be.fulfilled
        ).timeout(5000);
    });
    (SESSION_ID ? describe : describe.skip)('Get profile posts', () => {
        ...
        (PUBLIC_PROFILE ? it : it.skip)(
            'gets posts by ID',
            () => expect(client.getProfilePostsById(PUBLIC_PROFILE_ID, +GET_COUNT)).to.be.fulfilled
        ).timeout(5000);
        (PUBLIC_PROFILE_ID ? it : it.skip)(
            'gets posts by username',
            () => expect(client.getProfilePosts(PUBLIC_PROFILE, +GET_COUNT)).to.be.fulfilled
        ).timeout(5000);
    });
    ...
    describe('Get location', () => {
        let client;
        before(() => (client = new Instagram()));
        it('gets location by ID', () =>
            expect(client.getLocation(LOCATION_ID)).to.be.fulfilled).timeout(8000);
    ...
    describe('Search', () => {
        let client;
        before(() => (client = new Instagram()));
        (SEARCH_PROFILE ? it : it.skip)(
            'searches profile',
            () => expect(client.searchProfile(SEARCH_PROFILE)).to.be.fulfilled
        );
        (SEARCH_HASHTAG ? it : it.skip)(
            'searches hashtag',
            () => expect(client.searchHashtag(SEARCH_HASHTAG)).to.be.fulfilled
        );
        (SEARCH_LOCATION ? it : it.skip)(
            'searches location',
            () => expect(client.searchLocation(SEARCH_LOCATION)).to.be.fulfilled
        );
    });
});
